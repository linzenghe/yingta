...
<section class="photo_section">
    <div class="container">
        ...
        <div class="photo_nav text-center">
        	...
        	<a href="<?php echo $r['url'];?>"><span <?php if($j==1) { ?> class="current"<?php } ?>><?php echo $r['title'];?></span></a>
        	<?php $j++; ?>
        	<?php $n++;}unset($n); ?>
        	<?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
        </div>
        <div class="swipeboxEx justified-gallery">
        	<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=66535979b1fa5525ab0605b014f828cc&action=lists&catid=%24catid&order=id+ASC&num=1&moreinfo=1\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'lists')) {$data = $content_tag->lists(array('catid'=>$catid,'order'=>'id ASC','moreinfo'=>'1','limit'=>'1',));}?>  
			<?php $n=1;if(is_array($data)) foreach($data AS $v) { ?>
			<?php $n=1; if(is_array(string2array($v['morepic']))) foreach(string2array($v['morepic']) AS $pic_k => $r) { ?>
